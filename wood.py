...
with tab1:
    ...
    if st.session_state.analyzed_data:
        ...
        if u := st.chat_input("Frage..."):
            ...
            with st.spinner("..."):
                r = client.models.generate_content(model="gemini-2.0-flash", contents=f"Daten: {json.dumps(data)}\nFrage: {u}\nAntworte kurz.")
        ...
        for (ut, rev, los), grp in groups:
            ...
            with st.expander(final_title):
                ...
                if st.button("💾 Alles speichern (Notiz & Haken)", key=f"sv_t_{ut}"):
                    apply_batch_updates(ut, n_note, edited_p, match)
                    st.success("Gespeichert!"); st.rerun()
